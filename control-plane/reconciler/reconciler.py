...
class ReconciliationEngine:
        ...
        if action.action_type == ActionType.CREATE:
            ...
            for switch in self.switches:
                try:
                    ...
                    # OPTIMIZATION: Use the desired_state fetched at the start of the
                    # reconciliation cycle. This avoids a classic N+1 query problem where
                    # the database was queried for every VPC inside this loop.
                    for other_id, other_vpc in (
                        desired_state.get("vpcs", {}).items()
                    ):
                        if vpc_id == other_id:
                            continue
    ...
    def _process_pending_actions(self, result: ReconciliationResult, desired_state: Dict[str, Any]):
